...
the direction of the horizontal-velocity vector, and IMU acceleration
magnitude. Positive vertical speed is upward. Horizontal direction is the GNSS
course over ground, clockwise from north, and describes where the platform was
moving toward; the meteorological ``wind-from'' direction is approximately the
opposite bearing. It is omitted below 0.5~m/s and without a 3D fix, where course
is not meaningful. The red acceleration trace retains the maximum value in each
five-second plotting bin so short peaks remain visible beside the median trend.
...
\begin{enumerate}
  ...
  \item \textbf{Add in-flight radio-reconfiguration commands.} Implement acknowledged, bounded commands for controllable RF link parameters, especially transceiver transmit power and, where supported, data rate, spreading factor, bandwidth, and coding rate. Report the active configuration in telemetry, enforce electrical, thermal, and regulatory limits, and restore a known safe configuration after reset.
  \item \textbf{Enable wireless bulk-data recovery.} Add a command-controlled service for downloading the internal flight record when the payload cannot be physically reached, for example after landing in a tree or inaccessible area. Use a file manifest, fixed-size numbered chunks, CRC protection, retries, resumable transfer, and rate limiting so bulk download cannot starve command acknowledgement or essential live telemetry.
  \item \textbf{Evaluate an open 868-MHz telemetry network.} Assess a secondary path that forwards selected position, health, and recovery telemetry through an open or community LoRaWAN network in the European 868-MHz band to an Internet-accessible backend. Verify gateway coverage along the expected route, protocol and modem compatibility, payload and data-rate constraints, authentication and encryption, applicable duty-cycle and transmit-power limits, and behaviour when network coverage is unavailable. Treat this path as a complement to the direct TT\&C link and bulk-data recovery service, not as their replacement.
  \item \textbf{Develop a dual-antenna tracking ground station.} Use an omnidirectional antenna for initial acquisition and resilient reception in multipath, together with a higher-gain narrow-beam antenna on an automatic azimuth/elevation pointing system. Derive the pointing command from the CubeSat position carried in telemetry, retain manual override and a position-loss fallback, and log antenna selection, commanded bearing, and tracking error for post-test evaluation.
