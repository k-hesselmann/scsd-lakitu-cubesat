/* ============================================================================
 * coral.c  --  Coral Dev Board Micro UART5 interface  (STM32 OBC side)
 *
...
#include "cdh/coral.h"
#include "main.h"        /* huart5, huart2, Error_Handler */
#include "fatfs.h"       /* f_open, f_write, f_close, f_unlink, FIL, FR_OK */
#include <stdio.h>       /* snprintf */
#include <string.h>      /* strlen */

/* ---- External handles (declared in main.c) ------------------------------ */
/* Coral wired to UART5: PC12=TX (OBC->Coral cmds), PD2=RX (Coral->OBC data).
 * Both pins use AF8 -- configured by MX_UART5_Init() / HAL_UART_MspInit().  */
extern UART_HandleTypeDef huart5;   /* Coral UART (UART5)                    */
extern UART_HandleTypeDef huart2;   /* debug console (USART2, ST-LINK)       */

#define CORAL_HUART  huart5
...
static uint8_t coral_receive_frame(SensorData_t *dp)
{
    ...
    uint8_t hdr[16];
    if (HAL_UART_Receive(&CORAL_HUART, hdr, 16U, HDR_TIMEOUT_MS) != HAL_OK)
    {
    ...
    while (remaining > 0U)
    {
        ...
        if (HAL_UART_Receive(&CORAL_HUART, s_chunk, (uint16_t)take,
    ...
    if (HAL_UART_Receive(&CORAL_HUART, crc_bytes, 2U, CRC_TIMEOUT_MS) != HAL_OK)
    {
...
{
    /* huart5 must already be initialised by MX_UART5_Init() in main.c.       */
    s_uart_ready  = 1U;
    s_frame_count = 0U;
...
void Coral_Update(SensorData_t *dp)
{
    ...
    /* Non-blocking poll: try to read first SOF byte (timeout = 0) */
    uint8_t b0 = 0U;
    if (HAL_UART_Receive(&CORAL_HUART, &b0, 1U, 0U) != HAL_OK)
    {
        /* No data this tick -- normal between captures */
        return;
    }

    dbg_hex("[CORAL] Byte received: ", b0);

    if (b0 != CORAL_SOF_0)
    {
        dbg("[CORAL] !!! Not SOF_0 (0xAA) -- discarding spurious byte\r\n");
        return;
    }

    /* Got 0xAA -- read 0x55 with short deadline */
    uint8_t b1 = 0U;
    if (HAL_UART_Receive(&CORAL_HUART, &b1, 1U, SOF1_TIMEOUT_MS) != HAL_OK
        || b1 != CORAL_SOF_1)
    {
