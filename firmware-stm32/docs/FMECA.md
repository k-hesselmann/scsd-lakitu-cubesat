...
| C6 | Baro (MS5607) | plausible-but-wrong pressure | sensor drift, PROM corruption | wrong altitude | wrong phase transitions | GNSS absolute altitude vs reconstructed barometric pressure altitude (`baro_alt_m + persisted launch baseline`) when both sensors are valid **and GNSS has a current 3D fix**, disagreement >200 m sustained 5 s (first-pass threshold, pending flight calibration) — implemented in `cdh/sensor_validation.c` (`validateBaroGpsCrossCheck`), clears `baro_valid` so it flows through the ordinary C5 timeout monitor | prefer GPS altitude on disagreement | reinit; re-read PROM + CRC | Descent | 3 | L1 |
...
| Baro cross-check | C6 | current 3D-fix GNSS absolute altitude vs reconstructed barometric pressure altitude disagreement | 5 s | EQUIPMENT_BARO | baro reinit | CDH | FSM prefers GPS altitude |
