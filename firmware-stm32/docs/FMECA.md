...
|---|---|---|---|---|---|---|---|---|---|---|---|
| C1 | GPS (MAX-M10S) | no fix (module alive, responds on I2C) | environment, antenna, altitude limits | `gps_valid=1` (fresh transport), `fix_type!=3`, telemetry GNSS-fix-valid bit clear | phase detection degrades to baro | fix_type / data-present distinction (module supports it) | FSM and baro cross-check require a 3D fix | none — environmental, wait | Ascent | 2 | L1 |
...
| C4 | IMU (MPU-6050) | frozen/garbage values while ACKing | internal fault | `imu_valid=1` but data wrong | wrong phase transitions | plausibility check: impossible magnitude (>7.0g, while configured ±4g/axis) + stuck-value counter (3 identical consecutive reads) — implemented in `cdh/sensor_validation.c` (`validateImuPlausibility`), clears `imu_valid` so it flows through the ordinary C3 timeout monitor | ignore IMU when flagged | device reinit request | Launch | 3 | L1 |
...
| C6 | Baro (MS5607) | plausible-but-wrong pressure | sensor drift, PROM corruption | wrong altitude | wrong phase transitions | pre-flight only: GNSS absolute altitude vs reconstructed barometric pressure altitude (`baro_alt_m + persisted launch baseline`) when both sensors are valid **and GNSS has a current 3D fix**, disagreement >200 m sustained 5 s — implemented in `cdh/sensor_validation.c` (`validateBaroGpsCrossCheck`). The comparison is disabled after launch because dynamic pressure, independent sensor latency, and the ISA model can produce legitimate disagreement. | prefer GNSS for pre-flight diagnosis | inspect/re-read PROM before launch | Standby | 2 | L1 |
...
| IMU plausibility | C4 | impossible accel magnitude / stuck values | 3 cycles | EQUIPMENT_IMU | IMU reinit | CDH | FSM ignores IMU while flagged |
| Baro timeout | C5 | `baro_valid` | 3 cycles | EQUIPMENT_BARO | baro reinit, 10 s cooldown | CDH | with IMU fault → bus restart |
| Baro cross-check | C6 | Standby-only current 3D-fix GNSS absolute altitude vs reconstructed barometric pressure altitude disagreement | 5 s | EQUIPMENT_BARO | inspect before launch | CDH | disabled once flight begins |
