...
\bottomrule
\endfoot
MR-004 & Telemetry to 40~km altitude / 125~km worst-case slant range & LoRa physical layer and a requirements-level link-budget assumption are defined. & Range test open \\
...
FR-024 & European 868~MHz ISM-band operation & Flight registers select 868.000~MHz. & In band; frequency alignment open \\
...
FR-026 & SF9, 125~kHz bandwidth, coding rate 4/5 & Flight and ground configure explicit-header LoRa, SF9, BW125 and CR~4/5. & Registers implemented \\
PR-004 & At least 10~dB margin at 125~km & Section~\ref{sec:ttc-link-budget} recomputes the conditional margin. & Range test open \\
...
\midrule
Carrier frequency & 868.000~MHz (FRF \texttt{D9 00 00}) & Backend default 868.100~MHz; must be aligned \\
...
The 100~kHz center-frequency difference is a current integration defect. Both
values are inside the intended band, but transmitter and receiver should use
the same value. A shared configuration or explicit pre-flight register/readback
...
Appendix~\ref{app:ttc-wire} (Table~\ref{tab:ttc-wire-layout}) gives the
complete definition.
...
The requirements use 125~km as worst-case slant range. Free-space path loss is
\begin{equation}
L_{\mathrm{FS}} = 32.44 + 20\log_{10}(d_{\mathrm{km}})
                 + 20\log_{10}(f_{\mathrm{MHz}}),
\end{equation}
which gives 133.2~dB at 125~km and 868~MHz. The requirement assumes
$-137$~dBm receiver sensitivity at SF9/BW125. Antenna gains, cable,
polarization and installation losses, and measured sensitivity are absent, so
Table~\ref{tab:ttc-link-budget} is conditional rather than RF qualification.
...
Antenna gains (illustrative) & 0.0~dBi & 0.0~dBi \\
Free-space path loss & $-133.2$~dB & $-133.2$~dB \\
Other losses (illustrative) & $-3.0$~dB & $-3.0$~dB \\
Predicted receive power & $-122.2$~dBm & $-126.2$~dBm \\
Assumed sensitivity & $-137.0$~dBm & $-137.0$~dBm \\
\textbf{Illustrative margin} & \textbf{14.8~dB} & \textbf{10.8~dB} \\
\bottomrule
\end{tabular}
\caption{Conditional 125~km link budget using repository assumptions.}
\label{tab:ttc-link-budget}
\end{table}

With no extra losses, margins are 17.8~dB downlink and 13.8~dB uplink. With the
illustrative 3~dB allowance, uplink exceeds the 10~dB target by only 0.8~dB.
The requirements file states 26.5~dB without a reproducible calculation; the
zero-gain budget above does not reproduce it. Acceptance requires measured
sensitivity, characterized antennas/cables and representative range testing.

At SF9, BW125, CR~4/5, explicit header, CRC and eight-symbol preamble, symbol
time is 4.096~ms and 92-byte airtime is about 513~ms. At a strict 20~s period:
\begin{equation}
\frac{0.513}{20}\times100\% \approx 2.57\%.
\end{equation}
This conflicts with the 1\% channel-use assumption in FR-022/CR-005 before
