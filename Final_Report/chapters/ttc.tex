...
0.75~s so flight can process \textit{TxDone} and restore continuous RX, and
...
Command IDs are non-zero 16-bit values. A command is accepted once and a
...
\texttt{ACCEPTED}/\texttt{DUPLICATE} status echoed in valid telemetry. \texttt{REQ\_TELEMETRY} is currently the only
operational verb.
...
replace the latest valid spacecraft state. 