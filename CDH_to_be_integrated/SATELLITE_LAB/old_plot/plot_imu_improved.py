...
class IMUPlotter:
    def __init__(self, port, baudrate, max_points=100):
        ...
        self.fig.suptitle('IMU & Altimeter Data')
    ...
    def plot_data(self):
        ...
        if len(self.sample_num) > 0:
            ...
            # Plot altimeter
            ax3_pressure = self.ax3
            ax3_alt = self.ax3.twinx()

            ax3_pressure.plot(sample_list, list(self.pressure), label='Pressure', color='purple', linewidth=2, marker='.')
            ax3_pressure.plot(sample_list, list(self.temperature), label='Temperature', color='orange', linewidth=2, marker='.')
            ax3_alt.plot(sample_list, list(self.altitude), label='Altitude', color='brown', linewidth=2, marker='.')

            ax3_pressure.set_xlabel('Sample Number', fontsize=10)
            ax3_pressure.set_ylabel('Pressure (hPa) / Temp (°C)', fontsize=10, color='black')
            ax3_alt.set_ylabel('Altitude (m)', fontsize=10, color='brown')
            self.ax3.set_title('Altimeter (MS5607)', fontsize=12)
            ax3_pressure.grid(True, alpha=0.3)

            ax3_pressure.tick_params(axis='y', labelcolor='black')
            ax3_alt.tick_params(axis='y', labelcolor='brown')

            lines1, labels1 = ax3_pressure.get_legend_handles_labels()
            lines2, labels2 = ax3_alt.get_legend_handles_labels()
            ax3_pressure.legend(lines1 + lines2, labels1 + labels2, loc='upper left')
